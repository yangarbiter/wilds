...
def initialize_bert_transform(config):
    ...
    def transform(text):
        tokens = tokenizer(
            ...
            return_tensors='pt')
        if config.model in ['dp_bert-base-uncased', 'head_bert-base-uncased', 'bert-base-uncased']:
            x = torch.stack(
                (tokens['input_ids'],
...
def getBertTokenizer(model):
    if model in ['dp_bert-base-uncased', 'head_bert-base-uncased', 'bert-base-uncased']:
